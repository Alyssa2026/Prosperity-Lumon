...
class SquidInkStrategy(MarketMakingStrategy):
    def __init__(self, symbol: Symbol, limit: int) -> None:
        ...
        self.symbol = symbol
        self.limit = limit
        self.price_window = deque(maxlen=20)  # <- z-score window
        self.position = 0
        self.entry_price = 0
    ...
    def act(self, state: TradingState) -> None:
        ...
        if position == 0:
            if z < -1:
                self.buy(ask_price, 1)  # Long entry
                self.entry_price = ask_price
                self.position = 1
            elif z > 1:
                self.sell(bid_price, 1)  # Short entry
                self.entry_price = bid_price
                self.position = -1

        # === Exit ===
        elif position > 0 and abs(z) <= 0.1:
            self.sell(bid_price, 1)  # Close long
            self.position = 0

        elif position < 0 and abs(z) <= 0.1:
            self.buy(ask_price, 1)  # Close short
            self.position = 0
